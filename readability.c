...
int calc_sen(string text)
{
    ...
    return num_sentences; // this is the output of int calc_sen
}
...
int calc_words(string text)
{
    ...
    return num_words; // this is the output of int calc_words
}
...

{
    ...
    return num_letters; // this is the output of int calc_letters
}

int calc_grade( int sentences, int words, int letters)// we are going to pull from the output of functions
{

    float S = ((sentences / (float)words)*100); //finding the average of words per sen. We use float to keep the decimal points a
